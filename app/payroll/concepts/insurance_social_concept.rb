class InsuranceSocialConcept < PayrollConcept
  ...
  def evaluate(period, tag_config, results)
    ...
    payment_value = fix_insurance_round_up(
        BigDecimal.new(big_income*SocialInsuranceFactor(period, false))
    )
    InsuranceHealthResult.new(@tag_code, @code, self, {payment: payment_value})
  end

  def SocialInsuranceFactor(period, pens_pill)
    factor = 0.0
    if (period.year<1993)
      factor = 0.0
