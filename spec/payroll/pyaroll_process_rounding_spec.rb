...
describe 'Tax base rounding up' do
  ...
  it 'Tax base under 100 CZK should be round up to 1 CZK' do
    ...
    test_concept.tax_rounded_base( 99, @period).should ==  99
    test_concept.tax_rounded_base( 99.01, @period).should == 100
    test_concept.tax_rounded_base(100, @period).should == 100
  end

  it 'Tax base over 100 CZK should be round up to 100 CZK' do
    test_concept = TaxAdvanceBaseConcept.new(PayTagGateway::REF_TAX_ADVANCE_BASE.code, amount: 0)
    test_concept.tax_rounded_base(100.01, @period).should == 200
    test_concept.tax_rounded_base(101, @period).should == 200
  end
  ...
  it 'Tax advance from negative base should be 0 CZK' do
    ...
    test_concept.tax_adv_calculate(-1, -1, @period).should == 0
    test_concept.tax_adv_calculate(0, 0, @period).should == 0
  end

  it 'Tax advance should be round up to 1 CZK' do
    test_concept = TaxAdvanceConcept.new(PayTagGateway::REF_TAX_ADVANCE.code, amount: 0)
    test_concept.tax_adv_calculate(3333, 3333, @period).should == 500
    test_concept.tax_adv_calculate(2222, 2222, @period).should == 334
  end
  ...
  it 'Health insurance should be round up to 1 CZK' do
    ...
    test_concept.insurance_contribution(@period, 3333).should == 150
    test_concept.insurance_contribution(@period, 2222).should == 100
  end
